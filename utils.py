# -*- coding: utf-8 -*-
"""
Created on Tue May 12 20:39:21 2020
...
def _list_to_tf_dataset(dataset):
    ...
    return tf.data.Dataset.from_generator(
        ...
        output_shapes={'image': (32, 32, 3), 'label': (10)}
    )
...
def split_dataset(dataset, num_labelled, num_validations, num_classes):
    ...
    for example in iter(dataset):
        label = int(np.argmax(example['label']))
        counter[label] += 1
        ...
        unlabelled.append({
            'image': example['image'],
            'label': tf.convert_to_tensor(np.zeros(10), dtype=tf.float32)
        })
...
@tf.function
def my_map(x):
    return tf.map_fn(lambda batch: tf.image.random_crop(batch, size=(32, 32, 3)), x, parallel_iterations=cpu_count())
